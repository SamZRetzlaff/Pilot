...
class App extends React.Component {
  ...
  async handlePilotSubmit (event) {
    event.preventDefault()
    const currentUserResponse = await fetch(`http://localhost:8083/users/${this.state.currentPilotUser_ID}`)
    ...
    const currentPilotResponse = await fetch(`http://localhost:8083/pilots/${this.state.currentPilotUser_ID}`)
  ...
  async handlePilotSchedule (event) {
    event.preventDefault()
    const currentUserFlightsResponse = await fetch(`http://localhost:8083/flight/${this.state.currentPilotPilot_ID}`)
  ...
  handleFlightHoursUpdate = async () => {
    ...
    await fetch(`http://localhost:8083/pilots/${this.state.currentPilotUser_ID}`, {
      method: 'POST',
      headers: {
    ...
    .then(await (await fetch(`http://localhost:8083/pilots/${this.state.currentPilotUser_ID}`)).json())
  ...
  async handleViewFlight (event){
    event.preventDefault()
    const currentFlightResponse = await fetch(`http://localhost:8083/flight/singleflight/${this.state.current_flight_id}`)
  ...
  handleTakeoffUpdate = async () => {
    ...
    // await fetch(`http://localhost:8083/flight/singleflight/${this.state.current_flight_id}`, {
    //   method: 'POST',
    //   headers: {
    //     'Content-Type': 'application/json'
    //   },
    ...
    // })
    // .then(await (await fetch(`http://localhost:8083/flight/singleflight/${this.state.current_flight_id}`)).json())
